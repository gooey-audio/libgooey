...
- Decision: Use the Entity Ultra-Perc's twin-core, separately excited body/noise architecture as interface vocabulary, while labeling only signal paths the software actually implements.
  ...
  Date/Author: 2026-09-25 / Codex
- Decision: Add a separate `TwinCorePercVoice` engine instead of expressing the Ultra-Perc signal flow as a `ResonatorVoice` preset.
  ...
  Date/Author: 2026-09-25 / Codex
- Decision: Name the second engine `TwinCorePercVoice` and expose it through a standalone C ABI.
  Rationale: The name describes the synthesis topology without implying a product emulation. A standalone opaque handle, mono render function, presets, normalized parameters, discrete modes, triggers, MIDI note, deterministic seed, and ring limit let another app host the voice without depending on Rust internals or engine registration.
  Date/Author: 2026-09-26 / Codex
- Decision: Present both architectures through `PercussionEngine`, selected by `PercussionEngineKind`, with musical roles represented by `PercussionPreset`.
  Rationale: A UI dropdown should select peer signal topologies (`RoutingMatrix` or `TwinCore`) while kick, tom, snare, clap/hybrid, and metallic remain shared preset roles. The façade preserves the selected role when switching engines, exposes engine-specific parameter metadata, and is mirrored by `gooey_percussion_engine_*` FFI calls.
  Date/Author: 2026-09-26 / Codex
...
`gooey::instruments` exports the generic `ResonatorVoice` family and `TwinCorePercVoice`, `TwinCorePercConfig`, `TwinCorePercBodyMode`, and `TwinCorePercNoiseMode`. `PercussionEngine`, `PercussionEngineKind`, and `PercussionPreset` provide the stable topology/preset selection layer while retaining the concrete types for advanced editing and source compatibility. The generic `gooey_percussion_engine_*` C ABI supports dropdown-style topology selection, shared presets, topology-specific parameter discovery/editing, MIDI note, trigger, reset, and mono rendering. The advanced `gooey_twin_core_perc_*` ABI additionally exposes discrete modes, noise-only triggering, deterministic seed, activity query, and ring limiting; cbindgen includes both APIs in `include/gooey.h`. The implementation uses `Resonator`, `Exciter`, `StateVariableFilterTpt`, `Oversampler`, `SmoothedParam`, and `XorShift32`. The optional `visualization` feature includes `eframe` with its Glow renderer for the native graph lab. Neither audio graph allocates while rendering or uses a variable-size mode bank.

Revision note (2026-09-22): Created the implementation plan after the initial architecture and test surface were established; recorded the compatibility constraint that prevents replacing the public legacy parameter object directly. Updated after implementation with the noise-only lifecycle fix, final validation, deterministic legacy fixtures, the pre-existing Clippy result, and the follow-up native graph editor. Revised 2026-09-25 after replacing the minimal GLFW editor with a labeled `eframe`/`egui` lab and incorporating the Entity Ultra-Perc reference into the topology presentation. Revised 2026-09-26 to record the descriptive `TwinCorePercVoice` name, standalone host API, and unified engine-selection façade.
